 ...
 *           ============ 순서는 상관없다
 *           => ?no=1 ==> (int no)
 *           <input type=texe name=name> ==> (String name)
 *           
 *           => ?no=1&type=2 ==> (int a,int b)
 *           				 ==> (int no, int type) => 변수명으로 찾는다  (int type, int no) => 순서 상관 없이 써도 됨
 * 
 * 			@GetMapping
...
public class FoodController {
   ...
   @RequestMapping("food/find.do") // => get,post 동시 사용 ==> 페이징 : get , 검색 : post
